...
def build_nn_from_config(input_dim, output_dim, agent_name, config):
    ...
    apply_weight_norm = config["agents"][agent_name]["weight_norm"]
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, agent_name, config):
        ...
        self.net = build_nn_from_config(
            input_dim=state_dim, output_dim=action_dim, agent_name=agent_name, config=config
        )
...
class Critic_Q(nn.Module):
    def __init__(self, state_dim, action_dim, agent_name, config):
        ...
        self.net = build_nn_from_config(
            input_dim=state_dim + action_dim, output_dim=action_dim, agent_name=agent_name, config=config
        )
...
class Critic_V(nn.Module):
    def __init__(self, state_dim, agent_name, config):
        ...
        self.net = build_nn_from_config(input_dim=state_dim, output_dim=1, agent_name=agent_name, config=config)
