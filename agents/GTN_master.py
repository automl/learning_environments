...
class GTN_Master(GTN_Base):
    def __init__(self, config, bohb_id=-1, bohb_working_dir=None, file_dir=None, sh_file_workers=None):
        ...
        logger.info('Starting GTN Master with bohb_id {}'.format(bohb_id))
    ...
    def run(self):
        ...
        for it in range(1):
            ...
            if solved_flag and self.quit_when_solved:
                logger.info('ENV SOLVED')
                # self.bohb_next_run_counter += 1
                break

            logger.info('-- Master: rank transform' + ' ' + str(time.time() - t1))
            self.score_transform()
            logger.info('-- Master: update env' + ' ' + str(time.time() - t1))
            self.update_env()
