...
MBRL_PATHS = ["../mbrl_baseline_vary_2", "../mbrl_baseline_vary_4"]
...
def load_baseline(file_name, config):
    print(f"Loading baseline model {file_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent_name", type=str, help="must be in [ddqn_vary, duelingddqn_vary, td3_discrete_vary]", default="td3_discrete_vary")
    parser.add_argument('--pool_size', type=int, help='size of the multiprocessing pool', default=2)
    parser.add_argument('--agents_num', type=int, help='number of agents evaluated', default=200)
    ...
    experiment_name = f"mbrl_baseline_model_vary_2_best_models_{agent_name}_transfer_agents_num_{agents_num}_models_num_{len(MBRL_PATHS)}"
